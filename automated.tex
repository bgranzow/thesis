...
To alleviate the difficulties outlined above and to make adjoint-based
error estimation and mesh adaptation more accessible to solid mechanics
practitioners, we seek to develop a fully automated approach for parallel
adjoint-based error estimation and mesh adaptation. Specifically, we seek to
develop software that automates steps 1-6 outlined above based solely on the
...
space as used for the primal problem, and then enriched to a higher order
...
to solve local element problems to revover the strong form residual. We also
...
abstract weak formulation of our model problem: find $u \in \S$ such that
%
%% aut_abstract_primal
\begin{gather}
...
\eqref{eq:aut_abstract_primal} can be expressed as: find $u^H \in \V^H$ such
that
%
%% aut_abstract_stabilized_fem
\begin{gather}
\R_g(w; u) + \R_{\tau}(w; u) = 0 \quad \forall w \in \V.
...
additionally construct structures similar to traditional adaptive mesh
refinement (AMR) data structures with little implementation effort. Using
...
element-level residual vectors, Goal performs global finite element assembly
...
(FAD) \cite{griewank2008evaluating}. The Goal application utilizes the
...
to as $p$-enrichment. Additionally, the adjoint problem can be solved on a
...
onto the richer space $\S^h$ via interpolation. Let $J^h : \mathbb{R}^n \to
...
void AvgVM<ScalarT>::at_point(
  ...
  ScalarT vm = compute_von_mises<ScalarT>(sigma3x3);
...
that the distributino of degrees of freedom in this mesh closely
...
\cite{granzow2018adjoint}. For two and three dimensional problems in
...
presented in the reference \cite{granzow2018adjoint} is discussed. First,
...
function corresponding to pressures. The Cauchy stress tensor
is defined via a neo-Hookean constitutive relationship. For a complete
exposition, we refer the reader to \cite{granzow2018adjoint}.
...
behavior \cite{dong2017recovery}.
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.5\linewidth]{img/aut_glial_parts.png}
\caption{The initial parallel mesh partitioning for the
microglial cell example}
...
\subsection{An Industrial Application}
...
The authors acknowledge the support of IBM Corporation in the performance
of this research. The computing resources of the Center for Computational
Innovations at Rensselaer Polytechnic Institute are also acknowledged.
The development of tools used in this work was partly supported by the
U.S. Department of Energy, Office of Science, Office of Advanced
Scientific Computing Research, under award DE-SC00066117 (FASTMath SciDAC
Institute). The authos would like to thank Li Dong for providing the
discrete geometric model used for the microglial cell example and
Max Bloomfield for the geometric model used for the solder ball
example.
